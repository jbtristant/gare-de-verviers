...
void CommandStationClient::powerTrack(OnOff onOff, TrackType track) {
  if (track == TrackType::Both) {
    ...
    (onOff == OnOff::On) ? m_queue.push(F("<1 MAIN>"))
                         : m_queue.push(F("<0 MAIN>"));
  } else if (track == TrackType::Prog) {
    (onOff == OnOff::On) ? m_queue.push(F("<1 PROG>"))
                         : m_queue.push(F("<0 PROG>"));
  }
}
...
void CommandStationClient::configureTrackManager(char trackLetter,
                                                 TrackMode mode,
                                                 uint16_t cabId) {
  if (cabId == 0) {
...
void CommandStationClient::setLocoSpeed(uint16_t address, int8_t speed,
                                        Direction direction) {
  int8_t speedValue = 0;
  if (speed < 0) {
...
void CommandStationClient::_setLocoSpeed(uint16_t address, int8_t speed,
                                         Direction direction) {
  snprintf(m_generalWriteBuffer, sizeof(m_generalWriteBuffer), "<t %u %d %u>",
           address, speed, static_cast<uint8_t>(direction));
...
void CommandStationClient::setLocoFunction(uint16_t id, uint8_t function,
                                           OnOff onOff) {
...
void CommandStationClient::setLocoSpeedChangedCallback(
    void (*locoSpeedChanged)(uint16_t, int8_t, Direction)) {
  _locoSpeedChanged = locoSpeedChanged;
}

void CommandStationClient::setTurnoutStateChangedCallback(
    void (*turnoutStateChanged)(uint16_t id, TurnoutState state)) {
  ...
  if (type == 'R') {
    if (hasQuotes) {
      if (argCount >= 3) {
        ...
        if (found) {
          ...
          // Optionnel : Tu peux appeler une fonction pour mettre à jour ton
          // interface ici
        }
      }
    } else {
      ...
      m_locomotiveCount = 0;
      for (uint8_t i = 0; i < argCount && m_locomotiveCount < maxLocomotives;
           ++i) {
    ...
    if (hasQuotes) {
      ...
      for (uint8_t i = 0; i < argCount && m_turnoutCount < maxTurnouts; ++i) {
        ...
        for (uint8_t e = 0; e < m_turnoutCount && m_turnoutCount < maxTurnouts;
             ++e) {
        ...
        if (!found && m_turnoutCount < maxTurnouts) {
          m_turnouts[m_turnoutCount].initialize(this, id,
                                                TurnoutState::Undefined);
          m_turnoutCount++;
          askTurnoutInfo(id);
        }
      }
...
void CommandStationClient::handleLocoUpdate(char data[], byte size) {
  ...
  int matched = sscanf(data + 1, "%d %d %hhu %hhu", &cabid, &reg, &speedbyte,
                       &functionMap);
...
void CommandStationClient::handleTurnoutState(char data[], byte size) {
  ...
  if (matched == 2) {
    ...
    for (uint8_t i = 0; i < m_turnoutCount && m_turnoutCount < maxTurnouts;
         ++i) {
      if (m_turnouts[i].getId() == id) {
...
void CommandStationClient::taskProcessPendingCommand() {
  for (;;) {
    if (xSemaphoreTake(m_xStreamSemaphore, 100) == pdTRUE) {
      m_currentLocomotiveIndex =
          (m_currentLocomotiveIndex + 1) % maxLocomotives;
