...
void MainMenu::initialize(CommandStationClient* client, U8G2_SSD1306_128X64_NONAME_F_HW_I2C *screenMenu) 
{
...
void MainMenu::menuDown() 
{
    switch (m_currentContext) {
        case MAIN_MENU:
            m_menuIndex++;
            if (m_menuIndex >= m_mainMenuSize) m_menuIndex = 0;
            break;              
        case TRACK_MENU:
            m_menuIndex++;
        ...
        case DRIVING_MODE:
            ...
            Serial.print(F("Driving mode 'down' new speed: "));
            Serial.print(speed);
            Serial.print(F(" direction "));
            (direction == Direction::Forward) ? Serial.println(F("Forward")) : Serial.println(F("Reverse"));
...
void MainMenu::menuUp() {
    switch (m_currentContext) {
        case MAIN_MENU:
            if (m_menuIndex == 0) m_menuIndex = m_mainMenuSize - 1;
            else m_menuIndex--;
            break;  

        case TRACK_MENU:
            if (m_menuIndex == 0) m_menuIndex = m_commandStationClient->getTracksCount();
            else m_menuIndex--;
            break;
        
        case LOCO_MENU:
            if (m_menuIndex == 0) m_menuIndex = m_commandStationClient->getLocomotivesCount();
            else m_menuIndex--;
            break;
        ...
        case DRIVING_MODE:
            ...
            int8_t speed = locomotive->getSpeed();
            Direction direction = locomotive->getDirection();
            Serial.print(F("Driving mode 'up' speed: "));
            Serial.print(speed);
            Serial.print(F(" direction "));
            (direction == Direction::Forward) ? Serial.println(F("Forward")) : Serial.println(F("Reverse"));
            ...
            Serial.print(F("Driving mode 'up' new speed: "));
            Serial.print(speed);
            Serial.print(F(" direction "));
            (direction == Direction::Forward) ? Serial.println(F("Forward")) : Serial.println(F("Reverse"));
            locomotive->setSpeed(speed, direction);
        ...
        case TURNOUT_MENU:
            if (m_menuIndex == 0) m_menuIndex = m_commandStationClient->getTurnoutsCount();
            else m_menuIndex--;
            break;
...
void MainMenu::handlePress() {
    switch (m_currentContext) {
        case MAIN_MENU:
            switch (m_menuIndex) {
                case 0: m_currentContext = TRACK_MENU; break;
                case 1: m_commandStationClient->getLocomotive(m_menuIndex)->askLocoInfo(); m_currentContext = LOCO_MENU; break;
                case 2: m_commandStationClient->askTurnoutsT(); m_currentContext = TURNOUT_MENU; break;
                case 3: m_currentContext = STATUS_MENU; break;
                case 4: m_currentContext = CONFIGURATION_MENU; break;
                default: m_currentContext = MAIN_MENU; break;
            }
            m_menuIndex = 0;
            break;    
        ...
        case LOCO_MENU:
            if (m_menuIndex == m_commandStationClient->getLocomotivesCount()) {
                ...
                // pas de changement d'index, on le garde pour la loco et le retour au menu
            }
            break;
        ...
        case TURNOUT_MENU:
            if (m_menuIndex == m_commandStationClient->getTurnoutsCount()) {
                ...
                // pas de changement d'index, on le garde pour la sélection de l'aiguillage
...
void MainMenu::drawUI() {
    ...
    switch (m_currentContext) {
        case MAIN_MENU:
            ...
            Serial.println((m_menuIndex == 0) ? F("> Voies") : F("  Voies"));
            Serial.println((m_menuIndex == 1) ? F("> Locomotives") : F("  Locomotives"));
            Serial.println((m_menuIndex == 2) ? F("> Aiguillages") : F("  Aiguillages"));
            Serial.println((m_menuIndex == 3) ? F("> Statut réseau") : F("  Statut réseau"));
            Serial.println((m_menuIndex == 4) ? F("> Configuration") : F("  Configuration"));
            drawMainMenu();
            break;

        case TRACK_MENU:
            Serial.println(F("[ VOIES ]"));
            for(uint8_t i = 0; i < maxTracks; ++i) {
                ...
                Serial.print(F("  "));
                Serial.print(onOffToCString(track->getPower()));
                Serial.print(F("  "));
                Serial.println(trackModeToCString(track->getMode()));
            }
            Serial.println((m_menuIndex == m_commandStationClient->getLocomotivesCount()) ? "> [Retour]" : "  [Retour]");
            drawTrackMenu();
            break;

        // case TRACK_MODE:
        //     Serial.print(F("[ TRACK: "));
        //     Serial.print(m_commandStationClient->getTrack(m_menuIndex)->getName());
        //     Serial.println(F(" ]"));
        //     drawTrackMode();
        //     break;
        
        case LOCO_MENU:
            Serial.println(F("[ SELECTION LOCO ]"));
            for(uint8_t i = 0; i < m_commandStationClient->getLocomotivesCount(); i++) {
                Serial.println((m_menuIndex == i) ? String("> ") + m_commandStationClient->getLocomotive(i)->getName() : String("  ") + m_commandStationClient->getLocomotive(i)->getName());
            }
            Serial.println((m_menuIndex == m_commandStationClient->getLocomotivesCount()) ? "> [Retour]" : "  [Retour]");
            drawLocomotiveMenu();
            break;
        ...
        case DRIVING_MODE:
            Serial.print(F("[ PILOTAGE : "));
            Serial.print(m_commandStationClient->getLocomotive(m_menuIndex)->getName());
            Serial.println(F(" ]"));
            Serial.print(F(" Vitesse actuelle : "));
            Serial.println(m_commandStationClient->getLocomotive(m_menuIndex)->getSpeed());
        ...
        case TURNOUT_MENU:
            ...
            Serial.println((m_menuIndex == m_commandStationClient->getTurnoutsCount()) ? "> [Retour]" : "  [Retour]");
            drawTurnoutMenu();
            break;

        case STATUS_MENU:
            Serial.println(F("[ STATUT SYSTEME ]"));
            // Ici tu liras les vraies variables de ta centrale DCC-EX plus tard
            Serial.println(F(" SIGNAL DCC : ACTIF"));
            Serial.println(F(" COURANT    : 420 mA")); 
            Serial.println(F(" TENSION    : 14.8 V"));
            Serial.print(F(" LOCOS ACT. : ")); Serial.println(m_commandStationClient->getLocomotivesCount());
            Serial.print(F(" AIGUI ACT. : ")); Serial.println(m_commandStationClient->getTurnoutsCount());
            Serial.println(F("\n > CLIC POUR RETOUR"));
            drawStatusMenu();
            break;
...
void MainMenu::drawMainMenu() {
    ...
    // 2. Choix de la police pour le titre (ex: un peu plus grande ou en gras)
    ...
    for (uint8_t i = topIndex; i < m_mainMenuSize && ligneGraphique < m_screenMenuMaxVisibleLines; i++) {
        // Calcul de la position Y : ligne 1 à y=25, ligne 2 à y=37, etc. (pas de 12 pixels)
...
void MainMenu::dessinerMenuOptionnel() {
    ...
    m_screenMenu->drawHLine(0, 13, 128); // Petite ligne de séparation sous le titre
    ...
    for (int i = 0; i < 4; i++) {
        int yPos = 26 + (i * 12); // Calcule la position Y pour chaque ligne (espacement de 12 pixels)

        if (m_menuIndex == i) {
        // Si c'est l'élément sélectionné : on dessine un rectangle plein en fond
        m_screenMenu->setDrawColor(1);
        m_screenMenu->drawBox(0, yPos - 9, 128, 11); // x, y, largeur, hauteur
        
        // On écrit le texte en mode "inverse" (texte noir sur fond blanc)
...
void MainMenu::drawTrackMenu() {
    ...
    if (topIndex + m_screenMenuMaxVisibleLines < totalElements) {
        m_screenMenu->drawStr(122, 62, "v"); // Il y a des locos (ou le bouton retour) en-dessous
    }
...
//     m_screenMenu->setFont(u8g2_font_6x12_tf);
//     char titreBuffer[22];
//     snprintf(titreBuffer, sizeof(titreBuffer), "[ Voie %c ]", track->getName());
...
void MainMenu::drawTurnoutMenu() {
    ...
    // 4. PETIT BONUS : Indicateur visuel de défilement (Scrollbar simplifiée)
    // S'il y a plus d'éléments que de place, on dessine des flèches ou un indicateur
    if (topIndex > 0) {
...
void MainMenu::drawDrivingMode() {
    ...
    // int vitesseAbsolue = abs(vitesseReelle); // Transforme -40 en 40, et 40 reste 40
    ...
    m_screenMenu->setFont(u8g2_font_9x15_tf); 
    char vitBuffer[8];
    ...
    if (vitesseReelle < 0) vitesseReelle = 0;
    if (vitesseReelle > 126) vitesseReelle = 126; 

    // Calcul de la jauge : on "mappe" de 0 à 126 vers les 98 pixels disponibles
    ...
    m_screenMenu->setFont(u8g2_font_5x8_tf); 
...
void MainMenu::drawStatusMenu() {
    ...
    m_screenMenu->drawStr(75, 24, "ACTIF"); // Aligné à droite pour plus de clarté
    ...
    // Ligne 4 : Compteurs Locos et Aiguillages (combinés sur une ligne ou séparés)
    // Pour que tout entre, on peut formater une ligne avec les deux infos :
    ...
    m_screenMenu->setFont(u8g2_font_5x8_tf); // Police plus petite pour le bouton retour
